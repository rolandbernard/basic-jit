...
static void addLine(const char* line, int64_t line_number) {
    ...
    if(line_numbers[i] == line_number) {
        ...
        memmove(lines + i + 1, lines + i, num_lines - i);
        num_lines++;
        size_t len = strlen(line);
        lines[i] = malloc(len);
        memcpy(lines[i], line, len + 1);
...
static void removeLine(int64_t line_number) {
    ...
    if(line_numbers[i] == line_number) {
        free(lines[i]);
        num_lines--;
        memmove(lines + i, lines + i + 1, num_lines - i);
    }
}
...
static bool executeLine(const char* line) {
    MCGenerationData data = {
        ...
        .registers = 0,
        .line = 1,
        .run_function = runProgram,
        .new_function = removeAllLines,
        .list_all_function = listAllLines,
        .list_function = listLine,
    };
    bool end = false;
